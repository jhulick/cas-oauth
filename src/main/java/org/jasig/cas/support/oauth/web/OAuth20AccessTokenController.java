...
public final class OAuth20AccessTokenController extends AbstractController {
    ...
    protected ModelAndView handleRequestInternal(final HttpServletRequest request, final HttpServletResponse response)
        ...
        xMap.put("serviceId", service.getServiceId());
        xMap.put("tgt", ticketGrantingTicket.getId());
        m.put(uuid.toString(), xMap);

        response.setContentType("text/plain");
        final int expires = (int) (timeout - ((System.currentTimeMillis() - ticketGrantingTicket.getCreationTime()) / 1000));
        final String text = "access_token=" + uuid.toString() + "&expires=" + expires;
        log.info("\n\n*** text : {}", text);
